...
from utils.general import non_max_suppression_face, make_divisible, scale_coords, xyxy2xywh
...
class autoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        y = non_max_suppression_face(y, conf_thres=self.conf, iou_thres=self.iou, classes=self.classes)  # NMS (face: xyxy+conf+landmarks+cls)
...
class Detections:
    ...
    def __init__(self, imgs, pred, names=None):
        ...
        self.pred = pred  # list of tensors pred[0] = (xyxy, conf, cls) or face (xyxy, conf, landmarks, cls)
        self.names = names  # class names
        self.xyxy = pred  # xyxy pixels
        self.xywh = [self._to_xywh(x) for x in pred]  # xywh pixels (bbox cols only)
        self.xyxyn = [self._normalize_xyxy(x, g) for x, g in zip(self.xyxy, gn)]  # xyxy normalized
        self.xywhn = [self._normalize_xyxy(x, g) for x, g in zip(self.xywh, gn)]  # xywh normalized
    ...
    @staticmethod
    def _normalize_xyxy(x, g):
        # Face NMS returns 16 cols; only normalize bbox (first 4), not conf/landmarks/cls.
        if x is None or not len(x):
            return x
        y = x.clone()
        y[:, :4] = x[:, :4] / g[:4]
